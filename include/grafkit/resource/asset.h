...
namespace Grafkit::Asset {
	...
	public:
		ISerializedAsset(const std::type_index assetType, const std::vector<uint8_t>& data);
	...
	public:
		...
		[[nodiscard]] SerializedAssetPtr Load(std::type_index typeIndex, const std::string& assetName) const override
		{
			...
			ReadData(typeIndex, assetName, data);
		...
		{
			return Load(typeid(T), assetName)->DeserializeAs<T>();
		}
	};
