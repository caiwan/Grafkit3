...

{
	...
	public:
		...
		void Draw(Core::CommandBufferRef &commandBuffer, const uint32_t frameIndex, const uint32_t stageIndex) const;
	...
	private:
		...
		NodePtr m_root;
		std::vector<std::pair<NodePtr, MeshPtr>> m_nodes;
		std::vector<std::tuple<RenderStagePtr, std::vector<DrawCommand>>> m_drawCommands;
