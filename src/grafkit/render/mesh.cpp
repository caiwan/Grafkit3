...
void Grafkit::Mesh::Draw(const Core::CommandBufferRef &commandBuffer, uint32_t primitiveIndex) const
{
	assert(primitiveIndex < m_primitives.size());
	...
	vkCmdDrawIndexed(**commandBuffer, primitive.indexCount, 1, primitive.indexOffset, primitive.vertexOffset, 0);
}
